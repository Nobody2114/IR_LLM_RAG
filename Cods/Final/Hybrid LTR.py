...
for q_id, query_text in enumerate(queries):
    ...
    if len(sub_df) > 0:
        sub_df = sub_df.nlargest(min(1500, len(sub_df)), "similarity_score")
...
def load_ground_truth(path):
 ...
 # 【新增】强制转成数字，转不了的变成 NaN，顺便告诉你具体是哪几行有问题
...
for query_text, gt_path in GROUND_TRUTH_FILES.items():
    ...
    relevant_gt = ground_truth[ground_truth['true_relevance'] >= 3]  # 只算真正相关的
    ...
    print(f"仅针对真正相关视频的召回率: {true_recall:.2%}")
    
    scored = eval_df.merge(ground_truth, on='id', how='left')
    scored['true_relevance'] = scored['true_relevance'].fillna(0)
    scored = scored.sort_values('ltr_score', ascending=False).reset_index(drop=True)
    
    true_rel = scored['true_relevance'].to_numpy().reshape(1, -1)
    pred_score = scored['ltr_score'].to_numpy().reshape(1, -1)
    ...
    print(scored.sort_values('ltr_score', ascending=False)[['combined_text', 'url', 'ltr_score', 'true_relevance']].head(20).to_string())
